...
Deploy the Dockerized application from Project 3 on one cloud provider (AWS examples: VPC, EC2/compute, RDS, S3, IAM, ALB).

## Build

- Create VPC with public and private subnets across appropriate availability zones.
- Place an internet-facing load balancer in public subnets and run the Dockerized app on private EC2/compute where practical.
...
A browser reaches the Dockerized app through the load balancer; app reaches RDS; database is not publicly reachable. Record a simple architecture diagram and cost cleanup steps.
